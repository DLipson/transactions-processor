...
import { DefaultStatementProcessorFactory, StatementProcessorFactory } from "./StatementProcessorFactory";
...
export async function processStatementFiles(
  ...
  const statements = await Promise.all(validFiles.map((file) => processFile(file, factory, reader)));
...
function filterValidFiles(files: RawStatementFile[]): RawStatementFile[] {
  return files.filter((file) => file.file !== null && file.statementType !== null);
}
...
async function processFile(
  ...
  return new Statement(crypto.randomUUID(), file.statementType!, new Date(), items.statementItems);
}
